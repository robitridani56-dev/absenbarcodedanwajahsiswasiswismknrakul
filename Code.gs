...
function submitAttendance(barcode, jenis) {
  ...
  try {
    ...
    let status;
    if (jenis === "pulang") {
      const batasPulang = jadwalHariIni.jamPulang;
      status = jamSekarang < (batasPulang + ":00") ? "Pulang Cepat" : "Pulang";
    } else {
      const batasTerlambat = jadwalHariIni.jamTerlambat;
      status = jamSekarang <= (batasTerlambat + ":59") ? "Hadir" : "Terlambat";
    }
...
function getTodayLog() {
  ...
  // tapi statistik hadir/terlambat/pulang/total dihitung dari SELURUH
  // data hari ini supaya panel admin tidak salah hitung.
  const log = [];
  let totalHadir = 0, totalTerlambat = 0, totalPulang = 0;
  ...
  return {
    ...
    totalScan: totalHadir + totalTerlambat + totalPulang
  };
}
